...
export const createCategory = async(req,res)=>{
   try {
     ...
     const  slug=  slugify(req.body.slug).toLowerCase()
...
export const updateCategory = async (req, res) => {
    try {
      const { name} = req.body;
      const  slug=  slugify(req.body.slug).toLowerCase()
      const category = await Category.findByIdAndUpdate(req.params.id, { name:name,slug:slug,updatedAt: Date.now() }, { new: true });
